...
We consider the dataset with instances from multiple environment -- \\(D_{e}:=\left\{\left(x_{i}^{e}, y_{i}^{e}\right)\right\}_{i=1}^{n_{e}}\\). We want to learn a function \\(f(X)\approx Y\\).
